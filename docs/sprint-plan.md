...
- ⚠️ **Деплой:** публичный HTTPS-эндпоинт нужен уже сейчас, не к Week 6 — Mini App для показа профиля
  (решение 16 июля, см. «Новый скоуп») требует настоящего `https://` для `web_app` URL. Вариант для
  Sprint 1/2 без полноценного деплоя — dev-туннель (Cloudflare Tunnel/ngrok) поверх текущего long polling;
  решение по постоянному деплою (reverse proxy/Cloud Run) всё равно нужно не позже Week 6 из-за WHOOP webhooks.
