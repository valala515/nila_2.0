...
**6. Allowlist и структурированные events отсутствуют — allowlist закрыт 17 июля.** V0.2 (Tue 14 Jul)
включает allowlist и event names как часть Foundation. По факту на момент записи: бот отвечал любому
пользователю Telegram, событий как таковых (только `console.error` на unhandled) не было.
...
dogfood.
*Что сделано:* `ALLOWED_CHAT_IDS` (`src/config/env.ts`) + инвайт-only middleware в
`telegramHandlers.ts` (`isChatAllowed`) — по умолчанию пусто (без ограничений, решение пользователя на
переходный период), непустой список пропускает только перечисленные `chat.id`. Долгосрочно статический
список заменится на "разрешён, если привязан к аккаунту newmindstart" (см. план account linking, отдельный
ADR). Структурированные event names — по-прежнему открытый пункт, отдельно от allowlist.
...
- ✅ Allowlist — механизм реализован 17 Jul, список пока пуст (см. корректировку №6)
...
- ✅ **Allowlist:** механизм реализован 17 Jul (`ALLOWED_CHAT_IDS` в `env.ts` + инвайт-only
  middleware в `telegramHandlers.ts`, тест на `isChatAllowed`). Список пока сознательно пуст
  (решение пользователя) — ограничений нет, второй/третий тестер уже могут подключаться без
  правки `.env`. Долгосрочный гейт — не статический список, а "chat.id привязан к аккаунту
  newmindstart" (см. план account linking ниже, отдельный ADR); статический список остаётся
  как ручной override на переходный период.
